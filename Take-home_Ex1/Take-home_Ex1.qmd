...
The data set now contains only the data for non-functional water points, and it has 32,101 rows and 71 columns.
...
### Extracting Water Points from Different Water Sources

Let's extract the data of water points using well as the water source.

```{r}
wp_s_well <- wp_sf %>% filter(`#water_source_category` == "Well")
...
```{r}
wp_s_pw <- wp_sf %>% filter(`#water_source_category` == "Piped Water")
...
```{r}
wp_s_spring <- wp_sf %>% filter(`#water_source_category` == "Spring")

```

### Extracting Water Points using Different Technologies

Let's extract the data of water points using Mechanized Pump.

```{r}
wp_t_mp <- wp_sf %>% filter(`#water_tech_category` == "Mechanized Pump")
...
```{r}
wp_t_hp <- wp_sf %>% filter(`#water_tech_category` == "Hand Pump")
...
```{r}
wp_t_ts <- wp_sf %>% filter(`#water_tech_category` == "Tapstand")
...
```{r}
wp_t_unknown <- wp_sf %>% filter(`#water_tech_category` == "Unknown")
...
nga_wp <- nga %>% 
  ...
  mutate(wpt_s_well = lengths(
    st_intersects(nga, wp_s_well)
  )) %>%
  mutate(wpt_s_pipe = lengths(
    st_intersects(nga, wp_s_pw)
  )) %>%
  mutate(wpt_s_spring = lengths(
    st_intersects(nga, wp_s_spring)
  )) %>%
  mutate(wpt_t_mechanical = lengths(
    st_intersects(nga, wp_t_mp)
  )) %>%
  mutate(wpt_t_hand = lengths(
    st_intersects(nga,wp_t_hp)
  )) %>%
  mutate(wpt_t_tap = lengths(
    st_intersects(nga, wp_t_ts)
  )) %>%
  mutate(wpt_t_unknown = lengths(
    st_intersects(nga, wp_t_unknown)
  ))

```

Let's also calculate the percentage of different water sources and technologies.

```{r}
nga_wp <- nga_wp %>%
  mutate(wpt_s_well_pct = wpt_s_well / total_wpt) %>%
  mutate(wpt_s_pipe_pct = wpt_s_pipe / total_wpt) %>%
  mutate(wpt_s_spring_pct = wpt_s_spring / total_wpt) %>%
  mutate(wpt_t_mechanical_pct = wpt_t_mechanical / total_wpt) %>%
  mutate(wpt_t_hand_pct = wpt_t_hand / total_wpt) %>%
  mutate(wpt_t_tap_pct = wpt_t_tap / total_wpt) %>%
  mutate(wpt_t_unknown_pct = wpt_t_unknown / total_wpt)
